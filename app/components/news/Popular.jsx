  ...
  const fetchPopularNews = async () => {
    try {
      const request = await fetch(
        "https://newsapi.org/v2/top-headlines?country=us&apiKey=a6be9a0a722349e799d8ad7693692c6b"
      );
    ...
    <div
      className={` ${
        enabled
          ...
          : "flex flex-wrap justify-evenly gap-4"
      } `}
    >
      {newsDataLocal && newsDataLocal.length > 0
        ? newsDataLocal.map((item, index) => (
            <Card enabled={enabled} key={index} data={item} />
          ))
        : "No data found. Also Note: Requests from the browser are not allowed on the Developer plan, except from localhost. That's why unable to fetch data."}
    </div>
  );
};
