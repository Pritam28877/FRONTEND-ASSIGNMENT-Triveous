...
import {
  ...
  TOGGLEDIALOG,
  TOGGLESAVEDNEWS,
} from "../store/reducers/NewsDataReducer";
...
const NewsContent = () => {
  ...
  useEffect(() => {
    const index =
      savedNews.findIndex((item) => item?.title === dialogData?.title) || [];
    if (index === -1) {
                ...
                <Dialog.Panel className="w-full transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="flex flex-col justify-center items-start border-b-2">
                    ...
                    <span className="text-xs md:text-sm my-2 italic">
                      ...
                      >
                        {dialogData?.source?.name}
                      </a>
                    </span>
                    <img
                      src={dialogData?.urlToImage}
                      alt="img"
                      className="w-full h-[40vw] lg:w-6/12 lg:h-[30vw] mx-auto my-2"
                    />
                    <div className="flex justify-between items-center w-full text-xs md:text-sm my-2 italic">
                      <span>{dialogData?.author}</span>
                      <span>
                        {moment(dialogData?.publishedAt).format("ll")}
                      </span>
                    </div>
                  </div>
                    ...
                    <span className="flex">
                      ...
                      <StarIcon
                        ...
                        className={`w-6 h-6 cursor-pointer ${
                          favorite ? "text-yellow-500" : "text-slate-300"
                        }`}
                      />
                    </span>
